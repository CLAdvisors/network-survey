...
Public dashboard self-signup is controlled by `ALLOW_PUBLIC_SIGNUP`. It defaults to enabled only in local/dev/test when unset; set `ALLOW_PUBLIC_SIGNUP=false` for staging/production unless public account creation is explicitly desired. `AUTH_RATE_LIMIT_MAX`, `RESPONDENT_RATE_LIMIT_MAX`, `AUTHORING_PREAUTH_RATE_LIMIT_MAX`, and `AUTHORING_RATE_LIMIT_MAX` can tune API throttling for login/register, respondent-token endpoints, and large authoring requests before and after authoritative authentication.
