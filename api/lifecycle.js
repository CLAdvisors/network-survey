...
async function loadReminderData(client, survey, { lockRecipients = false, providerScope = '' } = {}) {
  ...
  const recipients = await client.query(
    `SELECT r.respondent_id,r.name,r.contact_info,r.uuid,r.lang
      ...
      WHERE r.survey_id=$1 AND r.can_respond=true AND r.response IS NULL
        ...
        AND NOT EXISTS(SELECT 1 FROM survey_email_deliveries prior JOIN survey_launches prior_launch ON prior_launch.id=prior.launch_id WHERE prior.survey_id=r.survey_id AND prior.respondent_id=r.respondent_id AND prior_launch.kind='reminder' AND prior.status='uncertain')
  ...
  return { recipients: recipients.rows, templates: templates.rows, suppressedExcludedCount, uncertainExcludedCount:Number(uncertain.rows[0]?.count||0) };
}
...
async function transitionSurvey(pool,user,surveyId,action) {
  ...
  try { await client.query('BEGIN'); await client.query(`SELECT pg_advisory_xact_lock(hashtextextended($1,0))`,[`survey-provider-boundary:${String(surveyId).toLowerCase()}`]); const survey=await loadAuthorizedSurvey(client,user,surveyId,minimum,'UPDATE');
    ...
    if(action==='close'||action==='archive') {
      await client.query(`UPDATE survey_email_deliveries SET status=CASE WHEN status IN ('pending','retry_wait','reminder_pending','reminder_retry_wait') THEN 'cancelled' ELSE status END,cancellation_requested_at=CASE WHEN status IN ('leased','reminder_leased') THEN now() ELSE cancellation_requested_at END,updated_at=now(),last_error_code=CASE WHEN status IN ('pending','retry_wait','reminder_pending','reminder_retry_wait') THEN $2 ELSE last_error_code END WHERE survey_id=$1 AND status IN ('pending','retry_wait','leased','reminder_pending','reminder_retry_wait','reminder_leased')`,[survey.id,action==='close'?'survey_closed':'survey_archived']);
