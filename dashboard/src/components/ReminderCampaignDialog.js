...
const ReminderCampaignDialog=({open,survey,onClose,onAccepted,unsavedChanges={},pendingOperations={}})=>{
  const [readiness,setReadiness]=React.useState(null);const [readinessSurveyId,setReadinessSurveyId]=React.useState(null);const [error,setError]=React.useState('');const [loading,setLoading]=React.useState(false);const [submitting,setSubmitting]=React.useState(false);const [key,setKey]=React.useState(newKey);const request=React.useRef(0);const requestInFlight=React.useRef(false);const id=surveyId(survey);const idRef=React.useRef(id);idRef.current=id;
  React.useEffect(()=>{if(!open||!id)return undefined;const version=++request.current;const controller=new AbortController();setLoading(true);setError('');setReadiness(null);setReadinessSurveyId(null);setKey(newKey());api.get(`/surveys/${id}/reminder-readiness`,{signal:controller.signal}).then(({data})=>{if(!controller.signal.aborted&&version===request.current){setReadiness(data);setReadinessSurveyId(id);}}).catch(err=>{if(!controller.signal.aborted&&version===request.current)setError(err.response?.data?.message||'Unable to preview reminder campaign.');}).finally(()=>{if(!controller.signal.aborted&&version===request.current)setLoading(false);});return()=>controller.abort();},[open,id]);
  const dirty=Object.values(unsavedChanges).some(Boolean);const pending=Object.values(pendingOperations).some(Boolean);const blockers=readiness?.blockers||[];const canLaunch=readinessSurveyId===id&&readiness?.canLaunch&&readiness.targetCount>0&&!dirty&&!pending&&!submitting;
  const submit=async()=>{if(!canLaunch||requestInFlight.current)return;const targetId=id;requestInFlight.current=true;setSubmitting(true);setError('');try{const {data}=await api.post(`/surveys/${id}/launches`,{kind:'reminder'},{headers:{'Idempotency-Key':key}});if(idRef.current===targetId)onAccepted?.(data);}catch(err){if(idRef.current===targetId)setError(err.response?.data?.message||'Unable to queue reminder campaign.');}finally{requestInFlight.current=false;setSubmitting(false);}};
