...
test('is read-only outside launched admin editing and exposes server conflicts',async()=>{api.get.mockResolvedValue({data:{templates:[]}});const view=render(<ReminderTemplateEditor surveyId="survey-1" editable={false}/>);expect(await screen.findByText(/editable by survey administrators only while the survey is launched/i)).toBeInTheDocument();expect(screen.getByLabelText(/Reminder subject/)).toBeDisabled();view.rerender(<ReminderTemplateEditor surveyId="survey-1" editable/>);});
