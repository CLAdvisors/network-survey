...
const Results = () => {
  ...
  const [useColoredEdges, setUseColoredEdges] = useState(true);
  ...
  // Update legend data when enabled questions change
  useEffect(() => {
  ...
  const getAnswerRows = () => {
    if (!selectedRespondent || !surveyData?.responses[selectedRespondent])
      return [];
            ...
            <CollapsibleSection title="Respondents">
              <Box sx={{ height: "300px", width: "100%" }}>
                <DataGrid
                  ...
                  pageSize={5}
                  onRowClick={(params) =>
                    handleRespondentSelect(params.row.name)
                  }
                  selectionModel={
                    selectedRespondent
                      ? [
                          respondentRows.find(
                            (r) => r.name === selectedRespondent
                          )?.id,
                        ]
                      : []
                  }
            ...
            <CollapsibleSection title="Settings">
              <Box sx={{ p: 2 }}>
                ...
                <FormControlLabel
                  control={
                    <Switch
                      checked={useColoredEdges}
                      onChange={(e) => {
                        setSelectedRespondent(null);
                        return setUseColoredEdges(e.target.checked);
                      }}
                    />
                  }
                  label="Show distinct edges"
                />
              </Box>
            </CollapsibleSection>
          </Box>
