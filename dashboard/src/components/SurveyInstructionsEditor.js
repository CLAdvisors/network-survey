...
const SurveyInstructionsEditor = ({ surveyId, readOnly = false, onDirtyChange, onOperationChange }) => {
  ...
  const handleSave = async () => {
    if (readOnly || loading || value === original || !begin(surveyId)) return;
  ...
  return (
    <Paper elevation={2} sx={{ p: 3, borderRadius: 2 }} aria-busy={loading || saving}>
      ...
      {readOnly && <Alert severity="info" sx={{ mb: 2 }}>Instructions are read-only after a survey has been launched.</Alert>}
      {notice && <Alert ref={noticeRef} tabIndex={-1} severity={notice.severity} sx={{ mb: 2 }}>{notice.message}</Alert>}
      <FormControl disabled={readOnly || loading || saving}>
      ...
      {mode === 'custom' && (
        <TextField
          ...
          onChange={(event) => setDraft(event.target.value)} disabled={loading || saving}
          InputProps={{ readOnly }}
          error={tooLarge}
          helperText={`${characterCount}/${limits.characters} characters; ${byteCount}/${limits.bytes} UTF-8 bytes`}
      ...
      <Box sx={{ mt: 2 }}>
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} justifyContent="flex-end">
          <Button variant="outlined" onClick={() => setDraft(original)} disabled={readOnly || loading || saving || !dirty}>Undo changes</Button>
          <Button variant="contained" onClick={handleSave} disabled={readOnly || loading || saving || !dirty || tooLarge}>
