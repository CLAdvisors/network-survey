...
For a ranking with any nonblank definition, every choice must persist an explicit, unique, canonical string `value` and a nonempty string `text`; definition-free rankings may use SurveyJS string shorthand. All draggable rankings are bounded to 100 choices per ranking and 1,000 choices per survey, so this version does not preserve unbounded legacy ranking schemas. Limits are 128 characters/512 UTF-8 bytes for values, 240 characters/1,024 bytes for labels, 10,000 characters/40,960 bytes per definition, 250,000 characters/512 KiB across definitions, and 2.5 MiB for the complete normalized survey schema. All schema keys and strings must also be PostgreSQL JSONB-compatible, and schemas are limited to 100 nesting levels and 100,000 JSON nodes. Respondent submissions use a route-specific 1 MiB request budget so the maximum definition-enabled ranking response fits while unrelated JSON routes retain the 100 KiB default; response JSON receives the same compatibility and structural checks before persistence. Larger authoring routes are IP/session throttled before authentication, then user-throttled and authenticated before parsing. Fast Entry is disabled because it cannot preserve multiline per-choice metadata.
