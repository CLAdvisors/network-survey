...
5. Before deploying, rehearse both changelog roots against a disposable snapshot
   ...
   the seven lifecycle, eight webhook, and two bulk-reminder changesets as pending. Apply them, rerun
...
3. Run master `validate`, `status --verbose`, and `update-sql`. Require exactly
   seven lifecycle plus eight webhook plus two bulk-reminder changesets pending; the CLA backfill must
   not be pending.
