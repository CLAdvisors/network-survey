...
5. Before deploying, rehearse both changelog roots against a disposable snapshot
   ...
   the seven lifecycle, eight webhook, and one editable-instructions reconciliation changesets as pending. Apply them, rerun
...
3. Run master `validate`, `status --verbose`, and `update-sql`. Require exactly
   seven lifecycle plus eight webhook plus one editable-instructions reconciliation changesets pending; the CLA backfill must
   not be pending.
