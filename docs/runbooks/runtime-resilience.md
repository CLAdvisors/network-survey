...
Before the one-time Terraform apply changes the target-group path, publish and deploy the `/live`-capable artifact to both existing hosts. The protected apply workflow reads the current target-group path and, only while it is not yet `/live`, mechanically compares registered targets with healthy InService ASG instances and runs the exact local `/live` contract through SSM on every target. That migration fails closed if any target is missing or incompatible. Once the target group already reports `/live`, the helper skips host/registration checks so Terraform remains available for one-host or two-host recovery. Do not bypass the migration preflight.
