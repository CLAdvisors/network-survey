...
CloudWatch Agent publishes EC2 memory, swap, root disk use/free inodes and ships PM2 plus kernel logs (not broad syslog content). Application EMF publishes process starts/heartbeat/RSS, V8 heap used/total, external, array-buffer bytes, event-loop lag, DB pool active/idle/waiting, and a bounded PostgreSQL dependency result. `external` includes `arrayBuffers`, so do not add those gauges together; use them to explain RSS rather than as an RSS identity.

Runtime alarm dimensions remain `Environment` and `Process`, aggregating the two prod-secondary hosts. RSS alarms use the maximum sample (the webhook warning is 224 MiB); start alarms sum starts across both hosts. The burst alarm remains more than three starts in 15 minutes. The webhook sustained alarm requires more than zero starts in at least three of the last eight one-hour periods (3-of-8); missing start data is not breaching. This catches recurring starts every two or three hours while a rolling deployment spanning at most two hourly buckets does not trigger it. Heartbeat alarms separately detect missing telemetry. Alarms also cover host pressure, EC2 status, event-loop lag, pool waits, direct DB dependency failure, kernel OOM/critical logs, ALB target health, ASG capacity, and RDS CPU/storage.
...
Before the one-time Terraform apply changes the target-group path, publish and deploy the `/live`-capable artifact to both existing hosts. The protected apply workflow binds the ASG and target-group identities from Terraform state, reads that exact target's current path with the Terraform role, and, only while it is not yet `/live`, switches to the narrower deploy role to compare registered targets with healthy InService ASG instances and run the exact local `/live` contract through SSM on every target. That migration fails closed if any target is missing or incompatible. Once the target group already reports `/live`, the helper skips host/registration checks so Terraform remains available for one-host or two-host recovery. Do not bypass the migration preflight.
...
Application rollback is the existing capability-checked immutable artifact rollback with a two-phase `/live` floor. During the prerequisite artifact rollout, while the ALB still checks `/health`, the host marker is absent so automatic restore may return to the current known-good non-`/live` release. The protected Terraform workflow inspects the state-bound target group after every apply attempt and, once AWS reports `/live`, stamps `/opt/service/alb-live-health-required` on every existing host; every new launch-template host creates the same marker before bootstrap. From that point, trusted rollback and on-host deploy/restore validation require `alb_live_health`, and deployment verifies `/live`. Rollback does not revert Terraform, schema, or runtime config. Never roll schema backward without its separate backup/capability procedure.
