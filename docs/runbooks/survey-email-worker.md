...
An older artifact without reminder-launch support may coexist with the additive schema before reminder work exists; the transitional shared-queue reminder capability (`reminder_provider_boundary: 1`) is explicitly rejected. Once any reminder campaign has been created, `validate-release-capabilities.js --database` permanently requires capability 2 because delayed or replayed reminder webhooks must remain kind-aware even after dispatch becomes terminal. Before the first reminder exists, an artifact without reminder-launch support remains a valid emergency rollback target; shared-queue capability 1 never is. To roll back after reminder history exists, stop claiming and sending, let no provider call remain in flight, and use only a capability-2-or-newer artifact.
