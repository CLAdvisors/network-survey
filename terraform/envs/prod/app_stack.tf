...
locals {
  ...
  survey_domains       = distinct(concat([var.survey_domain], var.legacy_survey_domains))
...
module "survey_frontend" {
  ...
  acm_certificate_arn  = aws_acm_certificate_validation.prod_survey_customer.certificate_arn
  enable_custom_domain = var.enable_frontend_custom_domains
  origin_id            = "S3-survey"
